_arrayInimigos = ["CUP_O_TK_INS_Soldier_TL", "CUP_O_TK_INS_Soldier_AT", "CUP_O_TK_INS_Soldier_AT", "CUP_O_TK_INS_Soldier_AT", "CUP_O_TK_INS_Soldier_GL"];
_arraySpawnDistancia = [400, 500]; // Array de Distância aleatoria para spawn dos inimigos
_gritoArabeArray = ["arab1","arab2","arab3","arab4","arab5","arab6","arab7","arab8","arab9","arab10"];
_numMaxInimigos = if (!isNil "qtdGruposAtaque") then [{qtdGruposAtaque},{8}]; // Máximo de inimigos que serão spawnados por grupo
...
if(isServer) then {
	...
	_arrived = false;
	while {!_arrived} do {
		if ((leader _group) distance (getMarkerPos "center_city") > 2000) then {
			if(debugar == 1) then {
				sleep 5;
			} else {
				sleep 60;
