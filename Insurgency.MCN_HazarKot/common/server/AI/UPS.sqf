...
while { _loop} do { 
	...
	if (count _members==0) then { 
		...
		// did the leader die?
		if (!alive _npc || primaryWeapon _npc == "") then { 
	...
	if !(_newpos) then { 
		...
		if (_waiting<=0) then { 
			...
			if ((!_swimming) && ((_dist<=_closeenough) || (_totmove<.2) || (_dammchg>0.01) || (curTimeontarget>ALERTTIME))) then { _makenewtarget=true; }; 
