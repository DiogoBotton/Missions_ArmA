...
sleep 0.1;
[] spawn {sleep 0.3;titleCut ["","BLACK IN",1]};

//Mouse input
CLY_spectate_cycle=0;
[] spawn {findDisplay 46 displayAddEventHandler ["MouseButtonDown","if ((_this select 1)==0) then {CLY_spectate_cycle=CLY_spectate_cycle+1};if ((_this select 1)==1) then {CLY_spectate_cycle=CLY_spectate_cycle-1}"]};
...
while {true} do {
	...
	_spectatees=[];
	_units=[];
	if (_playersonly and isMultiplayer) then {_units=playableUnits} else {_units=allUnits};
	if (isMultiplayer) then {
		{if ((side _x in _sides or count _sides==0) and (!_playersonly or isPlayer _x) and alive _x and _x!=player and isNil {_x getVariable "spectating"}) then {_spectatees=_spectatees+[_x]}} forEach _units
	} else {
		{if ((side _x in _sides or count _sides==0) and alive _x and _x!=player and isNil {_x getVariable "spectating"}) then {_spectatees=_spectatees+[_x]}} forEach _units
	};
	...
	if (count _spectatees>0 or isNull _spectatee) then {
		...
		if (_changed and count _spectatees>0) then {_spectatee=_spectatees select _i;_name=name _spectatee};
		...
		//Spectator hint
		_ai="";
		if (!isPlayer _spectatee and _spectatee!=_body) then {_ai=" (AI)"};
