...
if (_unit!=player or _weapontype==4 or (_magtype==16 and _magcount==1) or _bulletexplosive==1 or _weapon=="Throw") exitWith {};

_elevation=0.002;
_speed=(speed _bullet/3.6);
_weapondir=CLY_weapondir select 0;
if ((CLY_weapondir select 1)-(CLY_weapondir select 2)>0.085 or (CLY_weapondir select 1)+0.085<time) then {_weapondir=player weaponDirection _weapon};
if (_weapon in CLY_accuracyarray or count CLY_accuracyarray==0) then {_bullet setVelocity [(_weapondir select 0)*_speed,(_weapondir select 1)*_speed,(((_weapondir select 2)+_elevation)*_speed)]};