...
waitUntil {!isNil "CLY_defaultdensity"};
if (_amount==-1) then {_amount=CLY_defaultdensity};
...
while {_amount>0} do {
	...
	_activate=false;
	while {!_activate} do {
		if ({_logicpos distance [getPos vehicle _x select 0,getPos vehicle _x select 1,0]<_activatedist} count CLY_players>0) then {_activate=true};
	...
	for "_x" from 1 to _amount do {
		...
		while {_relocate} do {
			...
			if (!surfaceIsWater _pos and (getPos _logic select 2)==(getPosATL _logic select 2) and {_pos distance [getPos vehicle _x select 0,getPos vehicle _x select 1,0]<CLY_minspawndist} count CLY_players==0 and (!_ellipse or _pos distance _logicpos<_radius)) then {_relocate=false};
		};
		//Debug marker
		if (CLY_debug) then {
	...
	while {_amount>0 and {_logicpos distance [getPos vehicle _x select 0,getPos vehicle _x select 1,0]>_deactivatedist} count CLY_players<count CLY_players} do {
		{
